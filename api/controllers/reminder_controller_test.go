...
func TestReminderController_UpdateReminder_Success(t *testing.T) {
	...
	tests := []struct {
		name    string
		request string
		verify  func(t *testing.T, reminder *entities.Reminder)
	}{
		{
			name: "Full update",
			request: `{
			...
			verify: func(t *testing.T, reminder *entities.Reminder) {
				if reminder.Message != "Updated message" {
					t.Errorf("expected message to be 'Updated message', got %s", reminder.Message)
				}
				if reminder.IsActive != false {
					t.Error("expected isActive to be false")
				}
				if reminder.Recurrence.Type != entities.Weekly {
					t.Errorf("expected recurrence type to be Weekly, got %s", reminder.Recurrence.Type)
				}
			},
		},
		{
			name: "Partial update - message only",
			request: `{
				"message": "New message"
			}`,
			verify: func(t *testing.T, reminder *entities.Reminder) {
				if reminder.Message != "New message" {
					t.Errorf("expected message to be 'New message', got %s", reminder.Message)
				}
				if reminder.IsActive != existingReminder.IsActive {
					t.Error("isActive should not have changed")
				}
				if reminder.Recurrence.Type != existingReminder.Recurrence.Type {
					t.Error("recurrence should not have changed")
				}
			},
		},
		{
			name: "Partial update - activation status only",
			request: `{
				"isActive": false
			}`,
			verify: func(t *testing.T, reminder *entities.Reminder) {
				if reminder.Message != existingReminder.Message {
					t.Error("message should not have changed")
				}
				if reminder.IsActive != false {
					t.Error("expected isActive to be false")
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			updatedReminder := *existingReminder // Create a copy for each test
			...
			mock := &reminderUseCaseMock{
				...
				updateReminderFn: func(userID, reminderID int64, reminder *entities.Reminder) (*entities.Reminder, error) {
					if userID == 123 && reminderID == 1 {
						updatedReminder = *reminder // Store the update for verification
			...
			rw := httptest.NewRecorder()
			req := httptest.NewRequest(http.MethodPut, "/reminders/123/1", strings.NewReader(tt.request))
			req.Header.Set("Content-Type", "application/json")
			req.SetPathValue("user_id", "123")
			req.SetPathValue("reminder_id", "1")

			c.UpdateReminder(rw, req)
			...
			// Verify the update
			tt.verify(t, &updatedReminder)

			// Verify ID and UserID haven't changed
			if updatedReminder.ID != existingReminder.ID {
				t.Error("ID should not have changed")
			}
			if updatedReminder.UserID != existingReminder.UserID {
				t.Error("UserID should not have changed")
			}
			if !updatedReminder.CreatedAt.Equal(existingReminder.CreatedAt) {
				t.Error("CreatedAt should not have changed")
			}
		})
	}
}
